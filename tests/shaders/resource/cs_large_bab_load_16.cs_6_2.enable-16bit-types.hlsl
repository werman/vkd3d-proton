...
void main()
{
    ...
    fb.load1 = srv.Load<uint16_t>(byte_offset);

    uint16_t2 v2 = srv.Load<uint16_t2>(byte_offset);
    fb.load2_x = v2.x;
    fb.load2_y = v2.y;

    uint16_t3 v3 = srv.Load<uint16_t3>(byte_offset);
    fb.load3_x = v3.x;
    fb.load3_y = v3.y;
    fb.load3_z = v3.z;

    uint16_t4 v4 = srv.Load<uint16_t4>(byte_offset);
    fb.load4_x = v4.x;
    fb.load4_y = v4.y;
    fb.load4_z = v4.z;
    fb.load4_w = v4.w;
